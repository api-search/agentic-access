---
acting_count: 17
action_class_counts:
  acting: 17
  connected: 39
api_specs:
- filename: rocket-pool-auction-api-openapi.yml
  format: yaml
  label: Rocket Pool Auction API
  slug: rocket-pool-auction-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-auction-api-openapi.yml
- filename: rocket-pool-deposit-api-openapi.yml
  format: yaml
  label: Rocket Pool Deposit API
  slug: rocket-pool-deposit-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-deposit-api-openapi.yml
- filename: rocket-pool-megapool-api-openapi.yml
  format: yaml
  label: Rocket Pool Megapool API
  slug: rocket-pool-megapool-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-megapool-api-openapi.yml
- filename: rocket-pool-minipool-api-openapi.yml
  format: yaml
  label: Rocket Pool Minipool API
  slug: rocket-pool-minipool-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-minipool-api-openapi.yml
- filename: rocket-pool-network-api-openapi.yml
  format: yaml
  label: Rocket Pool Network API
  slug: rocket-pool-network-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-network-api-openapi.yml
- filename: rocket-pool-node-api-openapi.yml
  format: yaml
  label: Rocket Pool Node API
  slug: rocket-pool-node-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-node-api-openapi.yml
- filename: rocket-pool-oracledao-api-openapi.yml
  format: yaml
  label: Rocket Pool OracleDAO API
  slug: rocket-pool-oracledao-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-oracledao-api-openapi.yml
- filename: rocket-pool-protocoldao-api-openapi.yml
  format: yaml
  label: Rocket Pool ProtocolDAO API
  slug: rocket-pool-protocoldao-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-protocoldao-api-openapi.yml
- filename: rocket-pool-queue-api-openapi.yml
  format: yaml
  label: Rocket Pool Queue API
  slug: rocket-pool-queue-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-queue-api-openapi.yml
- filename: rocket-pool-rewards-api-openapi.yml
  format: yaml
  label: Rocket Pool Rewards API
  slug: rocket-pool-rewards-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-rewards-api-openapi.yml
- filename: rocket-pool-service-api-openapi.yml
  format: yaml
  label: Rocket Pool Service API
  slug: rocket-pool-service-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-service-api-openapi.yml
- filename: rocket-pool-settings-api-openapi.yml
  format: yaml
  label: Rocket Pool Settings API
  slug: rocket-pool-settings-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-settings-api-openapi.yml
- filename: rocket-pool-tokens-api-openapi.yml
  format: yaml
  label: Rocket Pool Tokens API
  slug: rocket-pool-tokens-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-tokens-api-openapi.yml
- filename: rocket-pool-wallet-api-openapi.yml
  format: yaml
  label: Rocket Pool Wallet API
  slug: rocket-pool-wallet-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/openapi/rocket-pool-wallet-api-openapi.yml
consequence_counts:
  physical: 3
  read: 39
  write: 14
description: Recommended x-agentic-access execution contracts, classified heuristically from the OpenAPI. A governance starting point for exposing this API to AI agents — review and bind audience per deployment. See research/curity/agentic-governance/.
human_in_the_loop: 0
kind: agentic-access
layout: agentic-access
method: generated
name: Rocket Pool Agentic Access
name_suffix: Agentic Access
notable_actions:
- action_class: acting
  consequence: physical
  human_in_the_loop: conditional
  method: POST
  path: /megapool/deposit
- action_class: acting
  consequence: physical
  human_in_the_loop: conditional
  method: POST
  path: /node/withdraw-rpl
- action_class: acting
  consequence: physical
  human_in_the_loop: conditional
  method: POST
  path: /queue/process
operation_count: 56
overview: 'Rocket Pool exposes 56 API operations that an AI agent could call, of which 17 are state-changing ''acting'' operations. This is a recommended x-agentic-access execution contract — the scope, audience, consequence tier, short-lived token constraints, and escalation each action should carry before it is handed to an autonomous agent.


  By consequence: 39 read, 14 write, and 3 physical.


  Contracts are classified heuristically from the provider''s OpenAPI and refresh on every APIs.io network build; audience is bound per deployment. The model follows Curity''s Access Intelligence (apidays Munich 2026). Browse every provider''s agent contracts at [agentic-access.apis.io](https://apis.io/agentic-access/).'
provider_name: Rocket Pool
provider_slug: rocket-pool
slug: rocket-pool-agentic-access
source_filename: rocket-pool-agentic-access.yml
source_heading: Agentic Access
source_url: ''
source_yaml: "generated: '2026-07-15'\nmethod: generated\nsource: openapi/rocket-pool-protocol-openapi.yml, openapi/rocket-pool-smart-node-openapi.yml\ndescription: Recommended x-agentic-access execution contracts, classified heuristically from\n  the OpenAPI. A governance starting point for exposing this API to AI agents — review and bind\n  audience per deployment. See research/curity/agentic-governance/.\nsummary:\n  operations: 56\n  by_action_class:\n    connected: 39\n    acting: 17\n  by_consequence:\n    read: 39\n    write: 14\n    physical: 3\n  human_in_the_loop_required: 0\noperations:\n- path: /deposit/pool\n  method: get\n  operationId: getDepositPool\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /minipools\n  method: get\n  operationId: listMinipoolsProtocol\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n\
  \      max-ttl: 3600\n    audit: none\n- path: /minipools/{address}\n  method: get\n  operationId: getMinipool\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /nodes\n  method: get\n  operationId: listNodes\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /nodes/{address}\n  method: get\n  operationId: getNodeDetails\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /nodes/{address}/minipools\n  method: get\n  operationId: listNodeMinipools\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /network/balances\n  method: get\n  operationId: getNetworkBalances\n  x-agentic-access:\n\
  \    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /network/fees\n  method: get\n  operationId: getNetworkFees\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /network/prices\n  method: get\n  operationId: getNetworkPrices\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /rewards/intervals\n  method: get\n  operationId: listRewardsIntervals\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /rewards/intervals/{index}\n  method: get\n  operationId: getRewardsInterval\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n\
  \    audit: none\n- path: /auction/lots\n  method: get\n  operationId: listAuctionLotsProtocol\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /tokens/reth\n  method: get\n  operationId: getRethState\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /tokens/rpl\n  method: get\n  operationId: getRplState\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /pdao/proposals\n  method: get\n  operationId: listProtocolDaoProposals\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /pdao/proposals/{id}\n  method: get\n  operationId: getProtocolDaoProposal\n  x-agentic-access:\n   \
  \ action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /odao/members\n  method: get\n  operationId: listOracleDaoMembersProtocol\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /settings/protocol\n  method: get\n  operationId: getProtocolSettings\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /wallet/status\n  method: get\n  operationId: getWalletStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /wallet/init\n  method: post\n  operationId: initWallet\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl:\
  \ 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /wallet/recover\n  method: post\n  operationId: recoverWallet\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /wallet/export\n  method: get\n  operationId: exportWallet\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /node/status\n  method: get\n  operationId: getNodeStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /node/sync\n  method: get\n  operationId: getNodeSync\n  x-agentic-access:\n  \
  \  action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /node/register\n  method: post\n  operationId: registerNode\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /node/rewards\n  method: get\n  operationId: getNodeRewards\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /node/stake-rpl\n  method: post\n  operationId: stakeRpl\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n\
  \    audit: required\n- path: /node/withdraw-rpl\n  method: post\n  operationId: withdrawRpl\n  x-agentic-access:\n    action-class: acting\n    consequence: physical\n    subject: required\n    audience: null\n    token:\n      max-ttl: 300\n      exchange: true\n      purpose-required: true\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /node/claim-rewards\n  method: post\n  operationId: claimNodeRewards\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /node/join-smoothing-pool\n  method: post\n  operationId: joinSmoothingPool\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl:\
  \ 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /node/leave-smoothing-pool\n  method: post\n  operationId: leaveSmoothingPool\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /minipool/status\n  method: get\n  operationId: listMinipools\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /minipool/{address}/stake\n  method: post\n  operationId: stakeMinipool\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n\
  \      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /minipool/{address}/exit\n  method: post\n  operationId: exitMinipool\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /minipool/{address}/close\n  method: post\n  operationId: closeMinipool\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /minipool/{address}/distribute-balance\n  method: post\n  operationId: distributeMinipoolBalance\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience:\
  \ null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /megapool/status\n  method: get\n  operationId: getMegapoolStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /megapool/deposit\n  method: post\n  operationId: depositMegapool\n  x-agentic-access:\n    action-class: acting\n    consequence: physical\n    subject: required\n    audience: null\n    token:\n      max-ttl: 300\n      exchange: true\n      purpose-required: true\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /megapool/exit-queue\n  method: post\n  operationId: exitMegapoolQueue\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n\
  \      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /queue/status\n  method: get\n  operationId: getQueueStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /queue/process\n  method: post\n  operationId: processQueue\n  x-agentic-access:\n    action-class: acting\n    consequence: physical\n    subject: required\n    audience: null\n    token:\n      max-ttl: 300\n      exchange: true\n      purpose-required: true\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /network/stats\n  method: get\n  operationId: getNetworkStats\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /network/node-fee\n\
  \  method: get\n  operationId: getNetworkNodeFee\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /network/rpl-price\n  method: get\n  operationId: getRplPrice\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /network/dao-proposals\n  method: get\n  operationId: getActiveDaoProposals\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /auction/status\n  method: get\n  operationId: getAuctionStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /auction/lots\n  method: get\n  operationId: listAuctionLots\n  x-agentic-access:\n    action-class: connected\n    consequence:\
  \ read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /auction/bid-lot\n  method: post\n  operationId: bidAuctionLot\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /odao/status\n  method: get\n  operationId: getOracleDaoStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /odao/members\n  method: get\n  operationId: listOracleDaoMembers\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /pdao/settings\n  method: get\n  operationId: getProtocolDaoSettings\n  x-agentic-access:\n    action-class: connected\n  \
  \  consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /pdao/voting-power\n  method: get\n  operationId: getProtocolDaoVotingPower\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /rewards/status\n  method: get\n  operationId: getRewardsStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /rewards/claim-all\n  method: post\n  operationId: claimAllRewards\n  x-agentic-access:\n    action-class: acting\n    consequence: write\n    subject: required\n    audience: null\n    token:\n      max-ttl: 900\n    escalation:\n      human-in-the-loop: conditional\n      triggers:\n      - abnormal\n      - high-value\n    audit: required\n- path: /service/version\n  method: get\n  operationId: getServiceVersion\n  x-agentic-access:\n    action-class:\
  \ connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n- path: /service/status\n  method: get\n  operationId: getServiceStatus\n  x-agentic-access:\n    action-class: connected\n    consequence: read\n    subject: optional\n    token:\n      max-ttl: 3600\n    audit: none\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/rocket-pool/refs/heads/main/agentic-access/rocket-pool-agentic-access.yml
summary_line: 56 operations · 17 acting
tags:
- Blockchain
- Ethereum
- Liquid Staking
- Proof of Stake
- DeFi
- Smart Contracts
- Node Operators
- Validator
- rETH
- RPL
- Minipool
- Decentralized Finance
- Decentralized Governance
- DAO
- Web3
---
